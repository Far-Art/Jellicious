...
export class ShoppingService {
    ...
    addProduct(product: Product): void {
        ...
        this.snackBar.open(`${product.name} נוסף לסל הקניות: כמות ${subject.value.quantity}`, undefined, {
            duration: this.snackDurationMs,
            direction: 'rtl'
        });
    }
    ...
    removeProduct(product: Product): void {
        if (this.cartMap.delete(product.id)) {
            this.snackBar.open(`${product.name} הוסר מסל קניות `, undefined, {
                duration: this.snackDurationMs,
                direction: 'rtl'
            });
        }
